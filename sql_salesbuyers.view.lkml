view: sql_salesbuyers {
  derived_table: {
    sql: SELECT t1.contactId, t1.quantityOrdered, t1.discountQuantity, t1.discountQuantityPercentage, t1.SalesBuyer,
      ...
      ((SUM (CASE WHEN discountAmount > 0 THEN 1 END)/SUM (quantityOrdered))>=0.75) AS SalesBuyer,
      FROM `body-fit-dev.orders.order_actual` order_actual,
      UNNEST (order_actual.orderLines) AS orderLines WHERE (quantityOrdered > 0) AND ((CAST(createdTimestamp AS DateTime)) BETWEEN DATE_ADD(CURRENT_DATE(), INTERVAL -18 MONTH) AND CURRENT_DATE())
